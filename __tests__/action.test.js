import { retrievePublishToken, generateMavenSettings, generateGradleProps, copyNpmConfig } from '../action.js';
...
test('can hit publish token endpoint', async () => {
    expect.assertions(1);
    try {
        await retrievePublishToken(expiredToken);
    } catch (e) {
...
test('can generate maven settins', async () => {
    await generateMavenSettings('/tmp', {
        username: 'test-user',
        token: 'token-123'
    });
